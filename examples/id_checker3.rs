...
use umatoi::api::event::CubeEvent;
...
pub async fn main() {
    ...
    let mut event_stream = cube.event_stream().await.unwrap();
    let notification_task = tokio::spawn(async move {
        while let Some(event) = event_stream.next().await {
            match event {
                CubeEvent::Id(id_data) => match id_data {
                ...
                CubeEvent::Motor(motor_response) => match motor_response {
